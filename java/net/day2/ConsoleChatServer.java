...
public class ConsoleChatServer implements Runnable {
	...
	boolean stop = false; // 채팅 지속 유무
	
	public ConsoleChatServer() {// 생성자 
		service();
	}// end of 생성자 ------------------------------
	
	public void service() {
		
		BufferedReader keyboardReader = null; // 서버자신이 클라이언트쪽으로 입력 내용을 보내기 위해 키보드로 부터 입력받은 내용을 읽어들이는 용도.
		PrintWriter	printWriter = null;	// 서버자신이 키보드로 부터 입력받은 내용을 클라이언트쪽으로 보내기 위한 용도.
		System.out.println("ConsoleChatServer 준비중...");
		
		try {// === ServerSocket 및 Socket 생성하기 ===
			...
			InetAddress inetip = socket.getInetAddress(); // 소켓에 연결된 클라이언트의 IP와 관련된 정보를 알 수 있는 InetAddress 객체를 리턴시켜준다.
			String ip = inetip.getHostAddress(); // 문자열 형태의 IP address 리턴시켜준다.
			...
			// === 클라이언트에서 보내오는 메시지를 계속 듣는 스레드 생성하여 동작시키기 ==
						...
						thr.start(); //메인은 보내는 스레드이고 이 스레드는 받아오는 스레드
			do { // === 서버에서 클라이언트쪽으로 메시지를 보내는 작업
				if(socket.isClosed())  // 클라이언트의 소켓이 닫힌 상태이라면
					//break;
					System.exit(0);
				else { // 소켓이 열려진 상태이라면
					String myMsg = keyboardReader.readLine(); // 키보드로 내(서버) 메시지 입력받기
			...
			System.exit(0); //메인이 아니기 때문에 return은 안됨
			} catch (IOException e) {
	...
	@Override
	public void run() { //클라이언트에서 보내오는 메시지를 계속 받아서 자기 콘솔에 출력 -- listener(리스너) 역할
		InputStream instream = null; // 클라이언트에서 보내온 데이터를 읽어들이기 위한 노드 스트림.
		BufferedReader bufferReader = null; // 클라이언트에서 보내온 데이터를 읽어들이기 위한 스트림으로 부터 읽어들이기위한 필터(보조) 스트림.
		
		try {
			instream = socket.getInputStream(); // 클라이언트에서 보내온 데이터를 읽어들이기 위한 노드 스트림.
			...
			String clientMsg = "";
			while(!stop) { 
				clientMsg = bufferReader.readLine(); // bufferReader 를 통해 클라이언트가 보내준 메시지를 한줄씩 읽어온다. 
				System.out.println("From Client>> "+clientMsg); // 콘솔창에 출력해준다.
			}// end of while---------------------------------
		} catch (IOException e) {
			// ***** 클라이언트에서 socket.close(); System.exit(0);이 발생하여 
			//       위의 bufferReader.readLine(); 부분 또는 IOException 이 발생하면 *****
