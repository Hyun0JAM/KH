...
public class ConsoleChatClient implements Runnable {
	...
	public ConsoleChatClient(String ip) { service(ip); } // 생성자 
	public void service(String ip) {
		try{
			socket = new Socket(ip, port); // === Socket 생성 (ip, port) ===
			...
			do { // === 클라이언트에서 서버쪽으로 메시지를 보내는 작업
				if(socket.isClosed())  // 소켓이 닫힌 상태이라면
					//break;
					System.exit(0);
				else { // 소켓이 열려진 상태이라면
					String myMsg = keyboardReader.readLine(); // 키보드로 내(클라이언트) 메시지 입력받기
	...
	@Override
	public void run() { // 서버에서 보내오는 메시지를 계속 받아서 자기 콘솔에 출력 -- listener(리스너) 역할
		InputStream instream = null; // 서버에서 보내온 데이터를 읽어들이기 위한 노드 스트림.
		BufferedReader bufferReader = null;  // 서버에서 보내온 데이터를 읽어들이기 위한 스트림으로 부터 읽어들이기위한 필터(보조) 스트림.
		try {
				...
				String serverMsg = "";
				while(!stop) { 
					serverMsg = bufferReader.readLine(); // bufferReader 를 통해 서버가 보내준 메시지를 한줄씩 읽어온다.
					System.out.println("From Server>> "+serverMsg); // 콘솔창에 출력해준다.
				}// end of while--------------------------------------------
			} catch (IOException e) {
...
}//end of class ConsoleConsoleChatClient //////////////////////////////////////////////////////
